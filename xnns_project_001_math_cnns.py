# -*- coding: utf-8 -*-
"""xNNs_Project_001_Math_CNNs.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Eghyp_Z0KJoDttP79MU8LiQgse_WTZFG
"""
...
class CNN2DConv_Layer(LayerInterface):
  """
    Neural Layer to convolve the image data to a new image dimension
    ...
    num_filters : int
        number of filters for the layer

    filters : NDArray[Float]
        filters for the convolution
    ...
    iterate_image(self, input_data):
      generator to yield all valid areas for the image
  ...
  def __init__(self, *dims):
    """
        ...
        Parameters
        ----------
            dims: shape of the filters matrix
    """
    self.input = None
    self.grad = None
    self.num_filters = dims[0]
    self.filters = np.random.rand(*dims) / 100.0
    self.input_shape = None
    self.output_shape = None

  def iterate_image(self, input_data):
    """
        Generator for producing valid area of the input_data

        Parameters
        ----------
            input_data: NDArray[float]
              image data being looped on

        Returns
        -------
            generator:
              used for proucin valid area of the input_data
    """
    _, height, width = input_data.shape
    for i in range(height - 2):
      for j in range(width - 2): 
        yield i, j, input_data[:,i:(i + 3),j:(j + 3)]
  ...
  def forward_pass(self, input_data):
    """
        ...
        -------
            NDArray[float]:
              image data convolve by the filters
    """
    self.input = input_data
    self.input_shape = input_data.shape
    pad_image = np.pad(input_data,((0,0),(1,1),(1,1)))
    _, height, width = input_data.shape
    output = np.zeros((self.num_filters,height,width))
    output_layer,_,_ = output.shape
    for x in range(output_layer):
      for i,j, region in self.iterate_image(pad_image):
        output[x,i,j] = np.sum(region * self.filters[x])
  ...
  def backward_prop(self, error):
    """
        ...
        -------
            NDArray[float]:
              returns error multiplied by image region
    """
    filter_errors = np.zeros(self.filters.shape)
    img_errors = np.zeros(self.input.shape)
    for i,j, region in self.iterate_image(self.input):
      for f in range(self.num_filters):
        filter_errors[f] += error[f,i,j] * region
        img_errors[:,i:(i + 3),j:(j + 3)] += error[f,i,j] * region
    self.grad = filter_errors 
    return img_errors
  ...
  def update_weight(self,learn_rate):
    """
        ...
        -------
            None
    """
    self.filters -= learn_rate * self.grad
    

class MaxPool_Layer(LayerInterface):
  """
    Neural Layer to perform max pooling on the image data
  ...
  def forward_pass(self,input_data):
    """
        ...
        -------
            NDArray[float]:
              max pooled image_data
    """
    self.input = input_data
    self.input_shape = input_data.shape
    pad_image = np.pad(input_data,((0,0),(1,1),(1,1)))
    layers, height, width = input_data.shape
    output = np.zeros((layers,height//2,width//2))
    for i,j, region in self.iterate_image(pad_image):
      output[:,i//2,j//2] = np.amax(region,axis=(1,2))
    self.output_shape = output.shape
    return output
  ...
  def backward_prop(self, error):
    """
        ...
        -------
            NDArray[float]:
              returns error with values where arg max appeared
    """
    pad_image = np.pad(self.input,((0,0),(1,1),(1,1)))
    error_hold = np.zeros(pad_image.shape)
    for i,j, region in self.iterate_image(pad_image):
      filters, height, width = region.shape
      amax = np.amax(region, axis=(1,2))
      for ff in range(filters):
        for hh in range(height):
          for ww in range(width):
            if region[ff,hh,ww] == amax[ff]:
              error_hold[ff,i+ hh,j + ww] = error[ff,i//2,j//2]
    error_hold = error_hold[:,1:-1,1:-1]
    return error_hold            
...
class Matrix_Multiplication_Layer(LayerInterface):
  ...
  def __init__(self, *dims):
    """
        ...
        Parameters
        ----------
            dims: shape of the weight matrix
    """
    self.input = None
    self.matrix = np.random.rand(*dims) / 100.0
    self.hold = None
    self.input_shape = None
    self.output_shape = None
...
class Bias_Layer(LayerInterface):
  ...
  def __init__(self, *dims):
    """
        ...
        Parameters
        ----------
            dims: shape of the weight vector
    """
    self.vector = np.random.rand(*dims) / 100.0
    self.hold = None
...
class CNNModel(object):
  ...
  def __init__(self, learn_rate):
    ...
    self.layers = [
      Division_Layer(),
      CNN2DConv_Layer(16,1,3,3),
      Bias_Layer(16,28,28),
      ReLu_Layer(),
      MaxPool_Layer(),
      CNN2DConv_Layer(32,16,3,3),
      Bias_Layer(32,14,14),
      ReLu_Layer(),
      MaxPool_Layer(),
      CNN2DConv_Layer(64,32,3,3),
      Bias_Layer(64,7,7),
      ReLu_Layer(),
      Vectorization_Layer(),
      Matrix_Multiplication_Layer(3136,100),
      Bias_Layer(1,100),
      ReLu_Layer(),
...
for x in range(10): # cycle through the epochs
    loss = 0
    ts = time()
    for idx,data in enumerate(tqdm(train_data[:100])): # cycle through the training data
        estimated_label = CNN.forward(data) # forward pass
        loss += cross_entropy_loss(estimated_label[0],train_labels[idx]) # loss
        # back prop
        CNN.backward(estimated_label - true_training_label[idx]) # estimated_label - true_training_label[idx] IS THE BACK PROPAGATION FOR SOFTMAX
        CNN.update() # weight update
    correct = 0
    for idx, Tdata in enumerate(tqdm(test_data[:100])): # cycle through the testing data
        estimated_test = CNN.forward(Tdata) # forward pass
        if test_labels[idx] == np.argmax(estimated_test): # forward pass
          correct += 1
    te = time()
    timeL.append(te-ts)
    ...
    print('epoch: {:d} | time: {:2.4f} sec | loss: {} | test accuracy: {}'.format(x+1,te-ts,loss/100,correct/100))
...
estimated_test_labels = [CNN.forward(Tdata) for idx, Tdata in enumerate(test_data[:100])]

# accuracy display
print('Final Accuracy of the Neural Net: {}'.format(accuracyL[-1]))
# final value
# plot of accuracy vs epoch
plt.title('accuracy & values vs epoch (100 train | 100 test)')
...
plt.title('time vs epoch (100 train | 100 test)')
...
for i in range(DISPLAY_NUM):
    ...
    ax[-1].set_title('True: ' + str(test_labels[i]) + ' CNN: ' + str(np.argmax(estimated_test_labels[i])))
